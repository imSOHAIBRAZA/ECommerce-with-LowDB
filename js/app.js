// Variables
const listaCursos = document.querySelector('#lista tbody');
...
function render() {
  ...
  setInterval(function(){ getToCart(); }, 1000);
  ...
  function showData(data) {
    ...
    data.map(function (e, i) {
      ...
      div.innerHTML = ` <div class="curso__item">
  <div class="curso__card">
    <img src="${e.url}" class="imagen__curso">
    <div class="info__card">
      <h4>${e.name}</h4>
      <p><span class="precio">${e.price}  </p>
      <button class="agregar-carrito" id="${e.id}" data-id="${e.id}"><i class="fa fa-cart-plus"></i>&nbsp;  ADD TO CART</button>
      
      </div>
  </div> <!--.card-->
</div>`;

      document.getElementById('cursor__container').appendChild(div);
  ...
  function EmptyCart() {
    while (listaCursos.hasChildNodes()) {
